...
class MiniApp {
    ...
    async verifyUser() {
        ...
        try {
            const userData = await this.fetchAPI(`/users/by_tg/${user.id}`);
    ...
    showStep(step) {
        ...
        // Update progress
        const stepOrder = ['service', 'calendar', 'time', 'confirm'];
    ...
    async loadCalendar() {
        if (!this.state.service)
            return;
        try {
            const response = await this.fetchAPI(`/slots/calendar?location_id=${this.state.locationId}&start_date=&end_date=`);
    ...
    async loadTimeSlots() {
        if (!this.state.service)
            return;
        try {
            const response = await this.fetchAPI(`/slots/day?location_id=${this.state.locationId}&service_id=${this.state.service.id}&date=${this.state.date}`);
