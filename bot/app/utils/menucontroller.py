...
class MenuController:
    ...
    async def show(self, message: Message, kb: ReplyKeyboardMarkup) -> None:
        ...
        Порядок для Android:
        1. Отправить новое меню (клавиатура сразу появляется)
        2. Удалить старый emoji бота (клавиатура уже есть)
        
        Сообщение пользователя НЕ удаляем — это триггерит IME.
        """
        chat_id = message.chat.id
        bot = message.bot
        ...
        # 3. Удалить старый emoji (клавиатура уже активна)
        if old_menu_id:
